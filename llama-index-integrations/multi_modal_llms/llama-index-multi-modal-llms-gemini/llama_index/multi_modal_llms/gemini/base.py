...
class GeminiMultiModal(Gemini):
    ...
    def complete(
        ...
        documents = [
            ImageDocument(image=node.resolve_image()) for node in image_documents
        ]
    ...
    def stream_complete(
        ...
        for s in self.stream_chat(messages=[msg], **kwargs):
            yield CompletionResponse(text=s.message.content or "")
    ...
    async def astream_complete(
        ...
        async for s in await self.astream_chat(messages=[msg], **kwargs):
            yield CompletionResponse(text=s.message.content or "")
